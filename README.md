...
2.  **Semantic Engine (Sentence-Transformers)**: Uses `Sentence-BERT` to process raw skill descriptions. It maps mechanical relationships (e.g., condition providers vs. consumers) into high-dimensional vector space (`description_embeddings.npz`).
...
*   **`description_embeddings.npz`**: Pre-computed NumPy arrays for skill semantics (ONNX).
